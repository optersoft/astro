// The company, as links: where every site sends a reader who wants Optersoft
// rather than the product in front of them. The footer's default columns and
// social row come from here, so a product site carries the same doors as
// optersoft.com without naming a single URL of its own.
...
/** One locale of the switcher: the same page, in another language. `name` is
 *  the language's name IN THAT LANGUAGE, never translated into the page's
...
export const OPTERSOFT = "https://optersoft.com";

/** The company's pages, English paths. optersoft.com itself passes its own
 *  localised columns; every other site gets these. */
export const OPTERSOFT_COLUMNS: Column[] = [
  {
    heading: "Optersoft",
    links: [
      { href: `${OPTERSOFT}/services`, label: "Services" },
      { href: `${OPTERSOFT}/technology`, label: "Technology" },
      { href: `${OPTERSOFT}/academy`, label: "Academy" },
      { href: `${OPTERSOFT}/apps`, label: "Apps" },
      { href: `${OPTERSOFT}/about`, label: "About" },
      { href: `${OPTERSOFT}/contact`, label: "Contact" },
    ],
  },
  {
    heading: "Legal",
    links: [
      { href: `${OPTERSOFT}/legal`, label: "Legal notice" },
      { href: `${OPTERSOFT}/privacy`, label: "Privacy policy" },
      { href: `${OPTERSOFT}/cookies`, label: "Cookies" },
    ],
  },
];

/** Official social profiles: name, URL, the icon's SVG path (24×24 viewBox).
 *  optersoft.com mirrors these in its Organization `sameAs` JSON-LD. */
export const SOCIALS: readonly [name: string, href: string, icon: string][] = [
  [
    "LinkedIn",
